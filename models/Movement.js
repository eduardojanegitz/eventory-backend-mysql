...
export const getAllMovement = async () => {
  const [movement] = await db.query(
    "SELECT M.MOVIMENTACAO_IN_ID as _id, I.ITEM_ST_NOME as item, L.LOCALIZACAO_ST_NOME as location, L2.LOCALIZACAO_ST_NOME AS newLocation , M.MOVIMENTACAO_ST_MOTIVO as reason, M.MOVIMENTACAO_ST_OBSERVACAO as observations, U.USUARIO_ST_ALTERNATIVO as user, M.MOVIMENTACAO_DT_CRIACAO as createdAt FROM EST_MOVIMENTACAO M LEFT JOIN EST_ITENS I ON I.ITEM_IN_ID = M.ITEM_IN_ID LEFT JOIN GLO_LOCALIZACAO L ON L.LOCALIZACAO_IN_ID = M.LOCALIZACAO_IN_ID LEFT JOIN GLO_LOCALIZACAO L2 ON L2.LOCALIZACAO_IN_ID = M.LOCALIZACAO_IN_ID_NOVA LEFT JOIN GLO_USUARIOS U ON U.USUARIO_IN_ID = M.USUARIO_IN_ID"
  );
  return movement;
};
